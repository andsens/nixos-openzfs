...
in
{
  ...
  config =
    lib.mkIf (config.openzfs.enable && (lib.length (builtins.attrNames autoDecryptPools) > 0))
      {
          ...
          destinations = lib.mapAttrsToList (pool: spec: {
            ...
            cmd = lib.getExe (
              pkgs.writeShellScriptBin "zfs-encrypt-key-tpm2.sh" ''
                ...
                systemd-creds encrypt --tpm2-device=auto --tpm2-pcrs=${lib.join "+" (map builtins.toString spec.tpmPCRs)} "$keyfile" "/etc/credstore.encrypted/${pool}.zfs-key"
              ''
            );
          }) autoDecryptPools;
        };
